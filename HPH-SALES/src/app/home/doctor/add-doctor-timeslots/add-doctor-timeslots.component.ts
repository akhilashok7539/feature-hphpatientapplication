...
@Component({
  selector: 'app-add-doctor-timeslots',
  templateUrl: './add-doctor-timeslots.component.html',
  styleUrls: ['./add-doctor-timeslots.component.css']
})
export class AddDoctorTimeslotsComponent implements OnInit {
    ...
    if (this.doctormodel.timing < this.doctormodel.timing2) {
      ...
      for (let j = 0; j < this.dataArray.length; j++) {
        if (this.session2 == true) {
          if (this.doctormodel.timing < this.doctormodel.timing2 && this.doctormodel.timing3 < this.doctormodel.timing4 ){
  ...
  handlesucess(data) {
    ...
    this.toaster.success('Timeslots added successfully');
    this.router.navigate(['/add-idproof']);
